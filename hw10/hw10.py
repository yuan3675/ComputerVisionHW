...
def LaplaceMask1(image, threshold):
    ...
    pixels = image.load()
    output = Image.new('1', (width-2, height-2), 1)

    for j in range(1, height-1):
        for i in range(1, width-1):
            result = pixels[i, j-1] + pixels[i-1, j] - 4 * pixels[i, j] + pixels[i+1, j] + pixels[i, j+1] 
            if result > threshold:
    ...
    for j in range(1, height-1):
        for i in range(1, width-1):
            ...
            for m in range(-1, 2):
                for n in range(-1, 2):
                    if m == 0 and n == 0:
                        result -= 8 * pixels[i+n, j+m]
                    else:
                        result += pixels[i+n, j+m]
...
def MinimumVariance(image, threshold):
    ...
    for j in range(1, height-1):
        for i in range(1, width-1):
            result = 2 * (pixels[i-1, j-1] + pixels[i+1, j-1] + pixels[i-1, j+1] + pixels[i+1, j+1]) \
            - (pixels[i, j-1] + pixels[i-1, j] + pixels[i+1, j] + pixels[i, j+1]) \
            - 4 * pixels[i, j]
            if (result/3) > threshold:
                output.putpixel((i-1, j-1), 0)
    return output
...
def LaplaceOfGaussian(image, threshold):
    width, height = image.size
    pixels = image.load()
    output = Image.new('1', (width-10, height-10), 1)
    ...
    for j in range(5, height-5):
        for i in range(5, width-5):
            ...
            for m in range(11):
                for n in range(11):
                    result += pixels[i+n-5, j+m-5] * mask[m][n] 
            if result > threshold:
...
def DifferenceOfGaussian(image, threshold):
    width, height = image.size
    pixels = image.load()
    output = Image.new('1', (width-10, height-10), 0)
